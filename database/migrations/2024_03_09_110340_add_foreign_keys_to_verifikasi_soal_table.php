...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('verifikasi_soal', function (Blueprint $table) {
            $table->foreign(['id_pengurus'], 'fk_vsoal_pengurus')->references(['id'])->on('pengurus_kbk')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign(['id_soal'], 'fk_vsoal_soal')->references(['id'])->on('soal')->onUpdate('cascade')->onDelete('cascade');
